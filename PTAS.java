...
public class PTAS {
    ...
    public static ArrayList<ArrayList<Integer>> maxAgree(ArrayList<ArrayList<Integer>> prob_matrix, int k, int sample_size, double epsilon) {
        ...
        int subset_size = (int) Math.ceil( ((double) num_nodes) / subset_num);
        ...
        for (int i = 0; i < subset_num; i++) {
            ...
            while (current_sample.size() < sample_size) {
                int nextVal = (int) (Math.random() * num_nodes);
        ...
        for (int i = 0; i < sample_sets.size(); i++) {
            ...
            HashSet<Integer> cur_sample = sample_sets.get(i);
            ArrayList<Integer> inputs = new ArrayList<Integer>();
            inputs.addAll(cur_sample);
            ArrayList<ArrayList<ArrayList<Integer>>> sample_clusterings = partition_k(inputs, k);
            for (int j = 0; j < sample_clusterings.size(); j++) {
                // find better clustering...
                ArrayList<ArrayList<Integer>> sample_clustering = sample_clusterings.get(j);
	...
	long hybridScoreTotal = 0;
        ...
        long pivotStart = System.currentTimeMillis();
        ArrayList<ArrayList<Integer>> pivot_result = maxAgree(prob_matrix, k, sample_size, 1.0);
        long pivotTime = System.currentTimeMillis() - pivotStart;
        pivotTimeTotal += (pivotTime / 1000.0);
        pivotTimes[j] = pivotTime;

        long fillStart = System.currentTimeMillis();
        ArrayList<ArrayList<Integer>> fill_result = maxAgree(prob_matrix, k, sample_size, 0.75);
        ...
        ArrayList<ArrayList<Integer>> rand_result = maxAgree(prob_matrix, k, sample_size, 0.5);
        ...
        ArrayList<ArrayList<Integer>> blend_result = maxAgree(prob_matrix, k, sample_size, 0.25);
        ...
        ArrayList<ArrayList<Integer>> fix = minDisagree(prob_matrix, k, sample_size);
